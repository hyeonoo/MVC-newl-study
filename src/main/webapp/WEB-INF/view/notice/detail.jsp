...
String url =  "jdbc:mysql://localhost:3306/newlec_study?useUnicode=true&characterEncoding=utf8";
		String sql = "SELECT * FROM NOTICE WHERE id=?";
			...
			String title = rs.getString("title"); 				/* 이것들이 Model이 된다. */ MVC1 방식
						...
						<table class="table">
							<tbody>
								...
								<tr>
									...
									<td>${n.writerId}</td>
									<th>조회수</th>
									<td><fmt:formatNumber value="${n.hit}"/></td>	<!-- 숫자 자릿수 끊어주기 -->
								</tr>
								<tr>
									<th>첨부파일</th>
									...
									<c:forTokens var = "fileName" items="${n.files}" delims="," varStatus="st">
										...
										<c:if test="${fn:endsWith(fileName , '.zip') }">						<!-- 파일 확장자에 대해서 조건처리하고 싶을땐 style 활용, endsWith() 적용이 안되...-->
											<c:set var="style" value="font-weight: bold; color: red;" />	
										</c:if>
										<a href="${fileName}" style="${style}">${fn:toUpperCase(fileName)}</a>	<!-- 출력되는 문자 대문자로!!하고 싶을떄 -->
										<c:if test="${not st.last}">  <!-- last -> boolean return, 마지막 / 빠짐 -->
										/
										</c:if>
									</c:forTokens>
									</td>
